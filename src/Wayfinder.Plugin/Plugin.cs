...
using Wayfinder.Shared;

namespace Wayfinder.Plugin;

/// <summary>
/// Wayfinder: a softly pulsing bead and a pointing glove that show which way to head, towards a target set by
...
public sealed class Plugin : IDalamudPlugin
{
    ...
    private readonly WindowSystem windowSystem = new("Wayfinder");
    ...
    private void OnUpdate(IFramework _)
    {
        ...
        {
            log.Error(ex, "Wayfinder: update failed");
        }
    }
    ...
    private void DrawUi()
    {
        ...
        {
            log.Error(ex, "Wayfinder: settings window failed");
        }
    ...
    private bool SetTargetFromIpc(uint territoryId, float x, float z, string label)
    {
        ...
        if (target is null)
        {
            log.Debug("Wayfinder.SetTarget refused: {Error}", error);
            return false;
        }
    ...
    private bool SetMapTargetFromIpc(uint territoryId, float mapX, float mapY, string label)
    {
        ...
        {
            log.Debug("Wayfinder.SetMapTarget refused for territory {Territory}", zone);
            return false;
        }
