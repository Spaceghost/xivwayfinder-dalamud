...
internal sealed class GloveTexture(ITextureProvider textures, IDataManager data, IPluginLog log)
{
    ...
    private void Resolve()
    {
        ...
        try
        {
            ...
            Status = path is null
                ? "the game's glove cursor was not found: drawing XivWayfinder's own glove"
                : $"the game's own glove: {path}, part {part.U},{part.V} {part.W}×{part.H}";
        }
        catch (Exception ex)
        {
            path = null;
            Status = "the game's glove cursor could not be read: drawing XivWayfinder's own glove";
            log.Warning(ex, "XivWayfinder: reading {Uld} failed", Glove.UldPath);
        }
    }
