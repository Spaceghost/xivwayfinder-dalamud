...
/// <summary>
/// Everything XivWayfinder reads from the game: the map flag (<c>AgentMap.FlagMapMarkers</c>), the journal
...
internal sealed unsafe class GameReader(IDataManager data, IAetheryteList attuned, IPluginLog log)
{
    ...
    public Target? Quest(uint playerTerritory, Vector3 player, ushort prefer = 0, QuestChoice choice = QuestChoice.Tracked)
    {
        ...
        {
            if (!warnedQuest)
                log.Warning(ex, "XivWayfinder: reading the journal failed; quest targets are off until the plugin reloads");
            warnedQuest = true;
        }
