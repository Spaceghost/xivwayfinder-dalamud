...
/// <summary>
/// The parsers that read text Wayfinder did not write (what the player types, what other plugins send over IPC)
/// must refuse bad input, never throw. WAYFINDER_FUZZ_SECONDS sets the budget (default 2, the nightly workflow
/// raises it); WAYFINDER_FUZZ_SEED replays a run.
/// </summary>
public sealed class FuzzTests
{
    ...
    {
        var seed = int.TryParse(Environment.GetEnvironmentVariable("WAYFINDER_FUZZ_SEED"), out var s) ? s : Environment.TickCount;
        var seconds = double.TryParse(Environment.GetEnvironmentVariable("WAYFINDER_FUZZ_SECONDS"), out var b) ? b : 2;
